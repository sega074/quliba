...
int main(int argc, char** argv) {
    ...
    QUMPoints <uint64_t,64> qp(try_atm_q); 

    QUMPoints <uint64_t,32> qp_i1(try_atm_q);

    QUMPoints <uint64_t,32> qp_i2(try_atm_q);

    QUMPoints <uint64_t,32> qp_i3(try_atm_q);
    ...
    std::vector <UT<uint64_t>> v(qp.getsz());
    int nel = 0;
    for (auto& vi :v){
        vi = std::make_unique<uint64_t>(nel++);
    }

    for (int i  = 0, j; i < 10 ; ++i){
        j = 0;
        while (qp.addElem(std::move(v[j++])) == nullptr && j < qp.getsz());
        j = 0;
        while ((v[j++] = std::move(qp.getElem())) != nullptr && j < qp.getsz());
    ...
    std::function <void(const int, const uint64_t,const uint64_t,const int , std::atomic<uint64_t>&, QUMPoints <uint64_t,32>&, QUMPoints <uint64_t,64>&)> 
            thread_send_x = [&](const int nid, const uint64_t elem,const uint64_t c_count, int try_atm_q, std::atomic<uint64_t>& count, QUMPoints<uint64_t,32>& qp_i, QUMPoints <uint64_t,64>& qp){
       
        while (bl_end == 2);

        UT< uint64_t> ret = nullptr;
        uint64_t        count_rp= 0;

        while (count < c_count && count_rp < c_count ){
            
            //if(ret = std::move(qp_i.getElem());ret == nullptr ) {
            if(ret = qp_i.getElem();ret == nullptr && ++count_rp < c_count) {    
            ...
            if (*ret == elem) {
                if (qp.addElem(std::move(ret)) == nullptr){
                    ++count;
                } 
            }  
        }
        std::cout << "end send " << elem  << " "<< "count repeat " << count_rp << std::endl;
        return;
    };
    ...
    auto thread_recv_q = [&](){
        ...
        while ( qp.elInQueue() > 0 || atm > 0) {    

            //ret = std::move(qp.getElem());
            ret = qp.getElem();
            if(ret == nullptr) {
                std::this_thread::sleep_for(std::chrono::microseconds(1));
                --atm;
    ...
    if ( (count1  != ucount1) || (count2 != ucount2) || (count3 != ucount3) ){
        //std::cout  << "first " << qp.p_beg.load().p.point;
        //std::cout << " end " << qp.p_end.load().p.point;
        std::cout << " elem " << qp.elInQueue() << std::endl;

        for (int i = 0; i < qp.vec_element_.size(); ++i){
            std::cout  << i << " " ;
            std::cout << (qp.vec_element_[i]!= nullptr ? *qp.vec_element_[i]:0);
            std::cout << std::endl;
        }
        return 1;
    }
